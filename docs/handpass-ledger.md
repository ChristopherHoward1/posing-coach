...
| `scripts/new-handoff.sh` | adapted (fixed in PR #6 / Issue #4) | Repo-root guard aborted on Windows Git Bash: `git rev-parse --show-toplevel` → `C:/Dev/...` but `pwd -P` → `/c/Dev/...`, so `REPO_ROOT != CURRENT_DIR`. Fixed by normalizing via `cd "$(git rev-parse --show-toplevel)" && pwd -P`. Axis: shell-env, with a template-design-flaw edge (a POSIX path assumption baked into the script). | one-time narrow fix, now landed. n=2 (identical check in `trigger-agent.sh`). | A portable repo-root check (`[ A -ef B ]`, or the `cd … && pwd -P` normalization used here) works on Windows too. Same *class* as the donor's `[[ -d .git ]]` worktree assumption. |
| `scripts/trigger-agent.sh` | adapted (fixed in PR #6 / Issue #4) | Identical repo-root guard as `new-handoff.sh` → same Windows abort; fixed identically. Also rejects a dirty tree (see `.gitignore` row). Separately, when *invoked* it dispatches `codex exec` into a Windows sandbox that is **edit-only** (see Part B) — orthogonal and unfixed. Axis: shell-env / template-design-flaw. | one-time; fix travelled with `new-handoff.sh`. | Fix both scripts' repo-root check together (done). The edit-only-sandbox limitation is a separate, unresolved constraint. |
