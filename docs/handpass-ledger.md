...
| `scripts/new-handoff.sh` | adapted-required (blocked on Windows) | Repo-root guard aborts on Windows Git Bash: `git rev-parse --show-toplevel` → `C:/Dev/...` but `pwd -P` → `/c/Dev/...`, so `REPO_ROOT != CURRENT_DIR`. Blocks handoff-branch creation. Axis: shell-env, with a template-design-flaw edge (a POSIX path assumption baked into the script). | **blocking**; one-time narrow fix. n=2 (identical check in `trigger-agent.sh`). | A portable repo-root check (`[ A -ef B ]`, or `cd "$(git rev-parse --show-toplevel)"`) would work on Windows too. Same *class* as the donor's `[[ -d .git ]]` worktree assumption. |
| `scripts/trigger-agent.sh` | adapted-required (blocked on Windows) | Identical repo-root guard as `new-handoff.sh` → same Windows abort; blocks wrapper-based agent dispatch. Also rejects a dirty tree (see `.gitignore` row). Axis: shell-env / template-design-flaw. | **blocking**; fix travels with `new-handoff.sh`. | Fix both scripts' repo-root check together. |
